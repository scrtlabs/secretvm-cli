...
import { VmDetailsApiResponse } from "../../types";

export async function vmStatusCommand(vmId: string): Promise<void> {
    if (!vmId || vmId.trim() === "") {
        console.error("Error: Internal VM Database ID is required.");
        return;
    }
    ...
    try {
        ...
        console.log(
            `Sending GET request to: ${apiClient.defaults.baseURL}${endpointPath}`,
        );
        ...
        if (response.data) {
            ...
            console.log(`  Status:         ${vm.status} (State: ${vm.state})`);
