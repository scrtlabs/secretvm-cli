...
import { encryptDockerCredentials, encryptSecrets } from "../../services/encryption";
...
async function waitForJobAndFetchVm(
    ...
    while (Date.now() < deadline) {
        ...
        if (job.status === "failed") {
            throw new Error(
                `VM update job failed: ${job.error || "unknown error"}`,
            );
        }
        if (onProgress) {
...
export async function editVmCommand(
    ...
    await handleCommandExecution(
        ...
        async (): Promise<AxiosResponse> => {
            ...
            // Resolve docker-compose: use provided file or fall back to current VM compose.
            ...
            if (dockerComposePath && dockerComposePath.trim() !== "") {
                ...
                dockerComposeBase64 = Buffer.from(
                    vmDetails.docker_file,
                ).toString("base64");
            }
            ...
            if (kms === "contract") {
                ...
                if (globalOptions.interactive) {
                    console.log(
                        `Job ${jobId} queued. Waiting for completion...`,
                    );
                }
            ...
            // Non-KMS path (dstack / GKMS / no KMS): RSA-encrypted docker credentials.
            ...
            if (secrets_plaintext && secrets_plaintext.trim() !== "") {
                const enc = await encryptSecrets(secrets_plaintext.trim());
                secretsEncrypted = enc.encryptedData;
                secretsKey = enc.encryptedAESKey;
            }
            ...
            const kmsProviderValue = kms ? (kmsMap[kms] ?? kms) : undefined;
        ...
        (data: AxiosResponse) => {
            if (globalOptions.interactive) {
                ...
                if (vm && vm.id) {
                    ...
                    table.push([vm.id, vm.vm_name, vm.vm_status]);
